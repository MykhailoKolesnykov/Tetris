...
C_62_figure_like_L_rotated::C_62_figure_like_L_rotated()
{
	length = 6;
	height = 2;
	bottom_y = 2;
...
void C_62_figure_like_L_rotated::Show()
{
...
void C_62_figure_like_L_rotated::Show_after_rotation(int rotated_left_x, int rotated_top_y)
{
	top_y = rotated_top_y;
...
bool C_62_figure_like_L_rotated::Move_down(int current_level)
{
	while (!_kbhit())
...
void C_62_figure_like_L_rotated::Erase()
{
	COORD position;
...
bool C_62_figure_like_L_rotated::Move_left()
{
	if (!Check_for_free_cell(left_x + 2, top_y)) return false;
...
bool C_62_figure_like_L_rotated::Move_right()
{
...
bool C_62_figure_like_L_rotated::Rotatable(int & rotated_left_x, int & rotated_top_y, bool & rotated_shift)
{

	if (!Check_for_free_cell(left_x, top_y) && !Check_for_free_cell(left_x, bottom_y + 1)) return false;

	if (!Check_for_free_cell(left_x + 2, top_y) && !Check_for_free_cell(left_x + 2, bottom_y + 1)) return false;

	if (!Check_for_free_cell(right_x, top_y - 1) && !Check_for_free_cell(right_x, bottom_y + 1)) return false;

	if (!Check_for_free_cell(left_x + 2, top_y) && !Check_for_free_cell(right_x + 1, top_y)) return false;
	...
	if (Check_for_free_cell(left_x + 2, top_y)) ++free_cell_counter;
