...
C_2_figure_like_S::C_2_figure_like_S()
{
	length = 6;
	height = 2;
	bottom_y = 2;
...
void C_2_figure_like_S::Show_after_rotation(int rotated_left_x, int rotated_top_y)
{
	top_y = rotated_top_y;
...
bool C_2_figure_like_S::Move_down(int current_level)
{
	while (!_kbhit())

	{
		...
		for (int i = 0; i < length - 4; ++i)
		{
			...
			COORD dwBufferCoord = { 0, 0 };
			COORD dwBufferSize = { 1, 1 };
			SMALL_RECT rect = { (short)(left_x + i + 4), (short)(bottom_y), (short)(left_x + i + 4), (short)(bottom_y) };
			ReadConsoleOutput(GetStdHandle(STD_OUTPUT_HANDLE), &ci, dwBufferSize, dwBufferCoord, &rect);
...
bool C_2_figure_like_S::Move_left()
{
	
		CHAR_INFO ci;
		COORD dwBufferCoord = { 0, 0 };
		COORD dwBufferSize = { 1, 1 };
		SMALL_RECT rect = { (short)(left_x), (short)(top_y), (short)(left_x), (short)(top_y) };
		ReadConsoleOutput(GetStdHandle(STD_OUTPUT_HANDLE), &ci, dwBufferSize, dwBufferCoord, &rect);

		if (ci.Char.UnicodeChar != ' ' && ci.Char.UnicodeChar != '.') return false;
	
		rect = { (short)(left_x - 1), (short)(bottom_y), (short)(left_x - 1), (short)(bottom_y) };
		ReadConsoleOutput(GetStdHandle(STD_OUTPUT_HANDLE), &ci, dwBufferSize, dwBufferCoord, &rect);
...
bool C_2_figure_like_S::Move_right()
{
	CHAR_INFO ci;
	COORD dwBufferCoord = { 0, 0 };
	COORD dwBufferSize = { 1, 1 };
...
bool C_2_figure_like_S::Rotatable(int & rotated_left_x, int & rotated_top_y, bool & rotated_shift)
{
...
bool C_2_figure_like_S::Exist_free_place_for_new_figure()
{
